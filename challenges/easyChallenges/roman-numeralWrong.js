...
while(true) {
  ...
  numberRemaining = numberRemaining - romanNumeralVals[closestRomanNumeral]
...
class RomanNumeral {
  ...
  toRoman() {
    let numberRemaining = this.number;
    let romanNumeralResult = [];
    while (true) {
      let closestRomanNumeral = returnClosestPresetNumeral(romanNumeralVals, numberRemaining);

      //add latest roman numeral to array and update number remaining
      if (numberRemaining > 0) {
        romanNumeralResult.push(closestRomanNumeral);
        numberRemaining -= romanNumeralVals[closestRomanNumeral];
      } else {
        romanNumeralResult.unshift(closestRomanNumeral);
        numberRemaining = romanNumeralVals[closestRomanNumeral] + numberRemaining;
      }

      if (numberRemaining === 0) return romanNumeralResult.join('');
    }
  }
}
